...
public:
    ...
    void elimination(Matrix* A, int rI, int rJ) {
        ...
        if (abs(this->curMatrix[rI][rJ]) < 1e-10) {
            this->curMatrix[rI][rJ] = 0;
        }
    }
};
...
int main() {
    ...
    if (pipe != NULL) {
        ...
        Matrix X = invATA * ATB;
        fprintf(pipe, "plot [-100 : 100] [-100 : 100] %lf*x**3 + %lf*x**2 + %lf*x**1 + %lf*x**0 , '-' using 1:2 with points\n");
        for (int i = 0; i < m; ++i) {
            double x = v[0][i];
            double y = 0;
            for (int j = 0; j <= n; ++j) {
                y += A.getCurMatrix()[i][j] * X.getCurMatrix()[j][0];
            }
            fprintf(pipe, "%f\t%f\n", x, y);
        }
        fprintf(pipe, "e\n");
